          ...
          <div className="cp-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 24 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              <div className="cp-card">
                <h3 style={{ fontSize: 16, fontWeight: 700, color: "#F5F0E8", marginBottom: 4 }}>{t("contactDetailsTitle")}</h3>
                <p style={{ fontSize: 13, color: "#9895B0", marginBottom: 20 }}>{t("contactDetailsSub")}</p>
                <div className="cp-info-row">
                  <div className="cp-icon-box" style={{ background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.25)" }}><MessageCircle size={18} style={{ color: "#22c55e" }} /></div>
                  <div>
                ...
                <div className="cp-info-row">
                  <div className="cp-icon-box" style={{ background: "rgba(208,91,55,0.15)", border: "1px solid rgba(208,91,55,0.25)" }}><Mail size={18} style={{ color: "#D05B37" }} /></div>
                  <div>
                ...
                <div className="cp-info-row">
                  <div className="cp-icon-box" style={{ background: "rgba(168,85,247,0.15)", border: "1px solid rgba(168,85,247,0.25)" }}><Instagram size={18} style={{ color: "#a855f7" }} /></div>
                  <div>
                ...
                <div className="cp-info-row">
                  <div className="cp-icon-box" style={{ background: "rgba(14,165,233,0.15)", border: "1px solid rgba(14,165,233,0.25)" }}><MapPin size={18} style={{ color: "#0ea5e9" }} /></div>
                  <div>
              ...
              <div className="cp-card">
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                  ...
                  <h3 style={{ fontSize: 16, fontWeight: 700, color: "#F5F0E8" }}>{t("contactHoursTitle")}</h3>
                </div>
                {[{ day: "Monday – Saturday", time: "10 AM – 8 PM", highlight: false }, { day: "Sunday", time: "Special requests only", highlight: true }].map(h => (
                ...
                <p style={{ fontSize: 12, color: "#9895B0", marginTop: 16, lineHeight: 1.6 }}>{t("contactUrgent")}</p>
              </div>

              <button onClick={() => window.open("https://wa.me/917428091729?text=Hi PosterBabu! I have a question.", "_blank")}
                ...
                <MessageCircle size={20} fill="currentColor" /> {t("contactWACta")}
              </button>
            </div>
            ...
            <div className="cp-card">
              {sent ? (
                <div style={{ textAlign: "center", padding: "48px 0" }}>
                  ...
                  <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 900, color: "#F5F0E8", marginBottom: 12 }}>{t("contactSentTitle")}</h3>
                  <p style={{ color: "#9895B0", fontSize: 16, lineHeight: 1.7, marginBottom: 28 }}>{t("contactSentSub")}</p>
                  <button onClick={() => setSent(false)} style={{ background: "rgba(208,91,55,0.15)", color: "#E87A57", border: "1px solid rgba(208,91,55,0.3)", padding: "12px 28px", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
                    {t("contactSentBtn")}
                  </button>
                </div>
              ) : (
                <>
                  <h3 style={{ fontSize: 20, fontWeight: 700, color: "#F5F0E8", marginBottom: 6 }}>{t("contactFormTitle")}</h3>
                  <p style={{ fontSize: 13, color: "#9895B0", marginBottom: 28 }}>{t("contactFormSub")}</p>
                  <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                    <div>
                      <label className="cp-label">{t("contactYourName")}</label>
                      <input required className="cp-input" placeholder={t("contactYourNamePH")} value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                    </div>
                    <div>
                      <label className="cp-label">{t("contactEmailPhone")}</label>
                      <input required className="cp-input" placeholder={t("contactEmailPhonePH")} value={formData.contact} onChange={e => setFormData({ ...formData, contact: e.target.value })} />
                    </div>
                    <div>
                      <label className="cp-label">{t("contactMessage")}</label>
                      <textarea required rows={5} className="cp-input" placeholder={t("contactMessagePH")} value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })} style={{ resize: "vertical" }} />
                    </div>
                    <button type="submit"
