...
const openWhatsApp = (plan = "") => {
  const msg = plan ? `Hi PosterBabu! I want to order the ${plan} plan.` : "Hi PosterBabu! I want to order a poster.";
...
const PricingPage = ({ navigate }) => {
  ...
  const PRICING_PLANS = [
    { name: "Starter", desc: "Perfect for small shops starting online", oldPrice: 499, newPrice: 399, features: [`8 ${t("pricePostersMonth")}`, t("priceFestival"), "Business promotions", t("priceSupport")], highlight: false, popular: false },
    { name: "Growth",  desc: "For businesses that post regularly",      oldPrice: 999, newPrice: 799, features: [`20 ${t("pricePostersMonth")}`, t("priceFestival"), "Product promotions", t("pricePriority")], highlight: true, popular: true },
    { name: "Pro",     desc: "For brands that want consistent content", oldPrice: 1999, newPrice: 1999, features: [`40 ${t("pricePostersMonth")}`, t("priceFestival"), t("priceCustom"), t("pricePriority")], highlight: false, popular: false },
  ];
    ...
    <div style={{ background: "#060517", color: "#F5F0E8", fontFamily: "'DM Sans', sans-serif", minHeight: "100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        .fire-btn{background:#D05B37;color:#F5F0E8;border:none;padding:14px;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:background 0.2s;width:100%;}
        .fire-btn:hover{background:#B84E2E;}
        .ghost-btn{background:rgba(6,5,23,0.25);color:#F5F0E8;border:1.5px solid rgba(245,240,232,0.25);padding:14px;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:opacity 0.2s;width:100%;}
        .ghost-btn:hover{opacity:0.85;}
        .slate-btn{background:#0C0A1E;color:#F5F0E8;border:1px solid #2E2B45;padding:14px;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;transition:border-color 0.2s;width:100%;}
        ...
        @media(max-width:768px){.entry-grid{grid-template-columns:1fr!important;}.plans-grid{grid-template-columns:1fr!important;}.plans-grid .plan-card.highlight{transform:none!important;}}
      `}</style>

      {/* HEADER */}
      <section style={{ padding: "72px max(24px,5vw) 64px", textAlign: "center" }}>
        <span className="fire-tag">{t("priceTag")}</span>
        <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(36px,5vw,56px)", fontWeight: 900, marginTop: 20, marginBottom: 16 }}>
          {t("priceTitle")} <span style={{ color: "#D05B37" }}>{t("priceTitleHL")}</span>
        </h1>
        <div className="divline" style={{ margin: "0 auto 20px" }} />
        <p style={{ color: "#9895B0", fontSize: 18, maxWidth: 480, margin: "0 auto" }}>{t("priceSub")}</p>
      </section>
      ...
      <section style={{ padding: "0 max(24px,5vw) 80px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 36 }}>
            <p style={{ fontSize: 11, fontWeight: 800, color: "#9895B0", textTransform: "uppercase", letterSpacing: "2px", marginBottom: 8 }}>{t("priceOneTimeTitle")}</p>
            <p style={{ color: "#9895B0", fontSize: 14 }}>{t("priceOneTimeSub")}</p>
          </div>
          <div className="entry-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            {ENTRY_OFFERS.map((offer, i) => (
              <div key={i} className={`price-card ${offer.highlight ? "highlight" : ""}`}>
                <div style={{ marginBottom: 24 }}>
                  ...
                  <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 900, color: "#F5F0E8", marginTop: 6, marginBottom: 16 }}>{offer.name}</h3>
                  <div>
                    <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 52, fontWeight: 900, color: "#F5F0E8", lineHeight: 1 }}>₹{offer.newPrice}</span>
                    <span style={{ fontSize: 18, textDecoration: "line-through", marginLeft: 10, color: offer.highlight ? "rgba(245,240,232,0.45)" : "#9895B0" }}>₹{offer.oldPrice}</span>
                  </div>
                </div>
      ...
      <section style={{ padding: "80px max(24px,5vw)", background: "#0C0A1E" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <span className="fire-tag">{t("priceMonthlyTitle")}</span>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(28px,4vw,44px)", fontWeight: 900, marginTop: 16, marginBottom: 10 }}>
              Never run out of <span style={{ color: "#D05B37" }}>content</span>
            </h2>
            <p style={{ color: "#9895B0", fontSize: 15 }}>{t("priceMonthlySub")}</p>
          </div>
          <div className="plans-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20, alignItems: "start" }}>
            {PRICING_PLANS.map((plan, i) => (
              <div key={i} className={`plan-card ${plan.highlight ? "highlight" : ""}`}>
                {plan.popular && (
                  <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", background: "#D05B37", color: "#F5F0E8", padding: "5px 18px", borderRadius: 100, fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1.5px", whiteSpace: "nowrap" }}>
                    {t("priceMostPopular")}
                  </div>
                )}
                <div style={{ textAlign: "center", marginBottom: 24 }}>
                  <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, fontWeight: 900, color: "#F5F0E8", marginBottom: 8 }}>{plan.name}</h3>
                  <p style={{ fontSize: 13, color: "#9895B0", marginBottom: 20, lineHeight: 1.6 }}>{plan.desc}</p>
                  <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 8 }}>
                    <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 48, fontWeight: 900, color: "#F5F0E8", lineHeight: 1 }}>₹{plan.newPrice}</span>
                    <div>
                      <div style={{ fontSize: 14, color: "#9895B0", textDecoration: "line-through" }}>₹{plan.oldPrice}</div>
                      <div style={{ fontSize: 12, color: "#9895B0" }}>{t("pricePerMonth")}</div>
                    </div>
                  </div>
                </div>
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 14, marginBottom: 28, flex: 1, paddingTop: 20, borderTop: "1px solid #2E2B45" }}>
                  {plan.features.map((f, j) => (
                    <li key={j} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, fontWeight: 500, color: "#9895B0" }}>
                      <CheckCircle size={16} style={{ color: "#D05B37", flexShrink: 0 }} />{f}
                    </li>
                  ))}
                </ul>
                {plan.highlight
      ...
      <section style={{ padding: "80px max(24px,5vw)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ background: "#D05B37", borderRadius: 24, padding: "40px 48px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 32, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: "1.5px", color: "rgba(245,240,232,0.7)" }}>Launch offer — founder pricing</span>
              </div>
              <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(22px,3vw,32px)", fontWeight: 900, color: "#F5F0E8", marginBottom: 8 }}>First 50 businesses get lifetime discounts</h3>
              <p style={{ color: "rgba(245,240,232,0.75)", fontSize: 14 }}>Lock in your rate before we raise prices. No catch.</p>
            </div>
            <div style={{ display: "flex", gap: 24, background: "rgba(6,5,23,0.2)", padding: "20px 32px", borderRadius: 20, border: "1px solid rgba(245,240,232,0.15)", position: "relative" }}>
            ...
            {FAQS.map((faq, i) => (
              <div key={i} className="faq-item">
                <button className="faq-q" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  <span>{faq.q}</span>
                  <ChevronDown size={18} style={{ color: "#D05B37", flexShrink: 0, marginLeft: 16, transition: "transform 0.25s", transform: openFaq === i ? "rotate(180deg)" : "rotate(0deg)" }} />
                </button>
                {openFaq === i && (
                  <div style={{ padding: "0 24px 20px", fontSize: 14, color: "#9895B0", lineHeight: 1.8, borderTop: "1px solid #2E2B45", paddingTop: 16 }}>{faq.a}</div>
                )}
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: "80px max(24px,5vw)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(28px,4vw,44px)", fontWeight: 900, marginBottom: 16 }}>
            {t("priceCtaTitle")} <span style={{ color: "#D05B37" }}>{t("priceCtaSub")}</span>
          </h2>
          <button onClick={() => openWhatsApp("Custom Design ₹149")}
            style={{ background: "#D05B37", color: "#F5F0E8", border: "none", padding: "16px 40px", borderRadius: 14, fontSize: 17, fontWeight: 800, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", display: "inline-flex", alignItems: "center", gap: 12, transition: "transform 0.2s" }}
            onMouseEnter={e => e.currentTarget.style.transform = "scale(1.04)"}
            onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}>
            <MessageCircle size={20} /> {t("priceCtaBtn")}
          </button>
        </div>
      </section>
    </div>
